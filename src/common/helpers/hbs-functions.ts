export const helpers = {
  ...
  "selected-option-multiple": (id: any, compareId: any, oldIds?: any[]) => {
    console.log("oldIds:", oldIds);
    if (oldIds && Array.isArray(oldIds)) {
      return oldIds.includes(id) ? "selected" : "";
    }

    if (Array.isArray(compareId)) {
      return compareId.includes(id.toString()) ? "selected" : "";
    }
